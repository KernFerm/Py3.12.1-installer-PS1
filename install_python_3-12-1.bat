@echo off
echo This script will install Python 3.12.1 on your system.
...
bitsadmin /transfer "PythonDownloadJob" /download /priority normal https://www.python.org/ftp/python/3.12.1/python-3.12.1-amd64.exe "%cd%\python-3.12.1.exe"

:: Check if the download was successful
if not exist "%cd%\python-3.12.1.exe" (
...
"%cd%\python-3.12.1.exe" /quiet InstallAllUsers=0 PrependPath=1 Include_test=0

:: Check the result of the installation
if %ERRORLEVEL% equ 0 (
    echo Python 3.12.1 has been installed successfully.
) else (
...
set "newuserpath=%userpath%;%LocalAppData%\Programs\Python\Python312\Scripts;%LocalAppData%\Programs\Python\Python312"
